...
public class CompassMarkerBar : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        for (int i = 0; i < markers.Length; i++)
        {
            ...
            float deltaAngle = Mathf.DeltaAngle(heading, markerAngle);
            float x = deltaAngle * pixelsPerDegree;

            RectTransform rect = markers[i].rectTransform;
            rect.anchoredPosition = new Vector2(x, 0f);

            bool isVisible = Mathf.Abs(x) <= panelWidth * 0.5f;
            markers[i].gameObject.SetActive(isVisible);
